<!DOCTYPE html>
<html>
<head>
  @include('user.head')
</head>
<body>
  @include('user.user-header')
  

<div class="container">
  <div class="row">
    <div class="col-md-9">
      <div class="home_content_wrap" style="padding-top: 40px">
        <div class="card">
          <!-- <i class="fa fa-ellipsis-h settings"></i> -->
          <div class="row single-page">
            <div class="col-md-12">
              <div class="single-page-cover-image"></div>
            </div>
            <div class="col-md-12">
             <div class="item_body">
                <div class="item_body_head">
                  <div class="row">
                    <img src="img/add.jpg">
                    <div class="col-md-10">
                      <div class="item_body_title"><a href="/single-page"><p>Post title</p></a></div>
                      <div class="item_body_meta">
                        <p>Posted by <span><a href="#">Mohammad Imran</a></span>, 26 March 2019 <span class="pull-right">Total view <i class="fa fa-eye" style="color: red;font-size: 16px;"></i> 500</span></p>
                        <p></p>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="item_body_content">
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                  <p>Intrinsicly evolve progressive products vis-a-vis focused web services. Energistically target wireless solutions before backend niches. Distinctively impact ethical leadership and covalent value. Distinctively impact ethical leadership and covalent value.</p>
                </div>

                <div class="item_body_footer">
                  <div class="row">
                    <div class="col-md-3">Like</div>
                    <div class="col-md-3">Share</div>
                    <div class="col-md-3">View</div>
                    <div class="col-md-3">Comment</div>
                  </div>
                </div>
             </div>
            </div>
          </div>
        </div>
      </div>

    </div>
    <div class="col-md-3">
      <p>right side bar</p>
    </div>
  </div>
</div>

 <script type="text/javascript" src="3rd-party/js/mdb.min.js"></script>
</body>
</html>